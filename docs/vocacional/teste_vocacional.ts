...
type Modalidade = "ead" | "semipresencial" | "presencial";

type Alternativa =
  | { texto: string; perfil: Perfil }
  | { texto: string; modalidade: Modalidade };
...
interface Curso {
  ...
  perfis?: Partial<Record<Perfil, number>>;
}
...
const pontuacao: Record<Perfil, number> = {
  tecnologia: 0,
  humanidades: 0,
  artes: 0,
  negocios: 0,
...
function embaralhar<T>(itens: T[]): void {
  for (let indice = itens.length - 1; indice > 0; indice--) {
    const posicaoAleatoria = Math.floor(Math.random() * (indice + 1));
    [itens[indice], itens[posicaoAleatoria]] = [
      itens[posicaoAleatoria],
      itens[indice],
    ];
  }
}
...
async function executar(): Promise<void> {
  const terminal = createInterface({ input: entrada, output: saida });

  try {
    console.log("=========================");
    console.log("Sistema Vocacional do IESB");
    console.log("=========================");
    console.log("Olá! Bem vindo ao sistema vocacional do IESB.");

    const nome = await terminal.question("Qual é o seu nome? ");
    console.log(`Prazer, ${nome}!`);
    console.log("Vou fazer algumas perguntas para");
    console.log("descobrir quais cursos combinam");
    console.log("mais com o seu perfil.");

    let modalidadeEscolhida: Modalidade | undefined;
    ...
    for (const pergunta of perguntas) {
      ...
      pergunta.alternativas.forEach((alternativa, indice) => {
        console.log(letras[indice], alternativa.texto);
      });

      let resposta = (await terminal.question("Escolha uma alternativa: "))
        .trim()
        .toUpperCase();

      while (!letras.includes(resposta) || resposta.length !== 1) {
        console.log(
          "Resposta inválida. Por favor, escolha uma alternativa entre A e F.",
        );
        resposta = (await terminal.question("Escolha uma alternativa: "))
          .trim()
          .toUpperCase();
      }

      const posicao = letras.indexOf(resposta);
      const alternativaEscolhida = pergunta.alternativas[posicao];

      if ("perfil" in alternativaEscolhida) {
        pontuacao[alternativaEscolhida.perfil] += 1;
      } else {
        modalidadeEscolhida = alternativaEscolhida.modalidade;
      }
    }

    const porcentagens = {} as Record<Perfil, number>;

    for (const perfil of Object.keys(pontuacao) as Perfil[]) {
      porcentagens[perfil] = Number(
        ((pontuacao[perfil] / 19) * 100).toFixed(2),
      );
    }

    const rankingPerfis = (Object.keys(porcentagens) as Perfil[]).sort(
    ...
    if (!modalidadeEscolhida) {
      throw new Error("A modalidade não foi escolhida.");
    }
    ...
    for (const curso of cursos) {
      ...
      if (curso.modalidades.includes(modalidadeEscolhida)) {
        console.log("Curso compatível com a modalidade escolhida.");
      } else {
        console.log("Curso incompatível com a modalidade escolhida.");
      }
    }
  } finally {
    terminal.close();
...
executar().catch((erro: unknown) => {
  ...
  console.error("Erro ao executar o sistema:", mensagem);
  process.exitCode = 1;
});